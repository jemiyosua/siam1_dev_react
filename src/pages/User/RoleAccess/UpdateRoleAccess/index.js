...
const UpdateRoleAccess = () => {
    ...
    const getDetailRole = () => {
		...
		.then((data) => {
			...
			if (data.ErrCode === "0") {
                setRoleName(data.Result[0].RoleName)
			} else {
				if (data.ErrCode === "2") {
		...
		<div>
            ...
            <div>
                <button role="tab" aria-controls="merchant-list">
                    <div style={{ color:'#004372', fontSize:16, fontWeight:'bold' }}>Update Role Access</div>
                </button>
            </div>
...
export default UpdateRoleAccess;